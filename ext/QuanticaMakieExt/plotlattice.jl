...
@recipe(PlotLattice) do scene
    Theme(
        ...
        sitecolor = missing,                # site shader, accepts (i, r) -> float
        siteopacity = missing,              # site shader, accepts (i, r) -> float
        maxsiteradius = 0.5,
        siteradius = 0.25,                  # site shader, accepts (i, r) -> float
        siteborder = 2,
        siteborderdarken = 0.6,
        sitedarken = 0.0,
        sitecolormap = :Spectral_9,
        hopcolor = missing,                 # hop shader, accepts ((i,j), (r,dr)) -> float
        hopopacity = missing,               # hop shader, accepts ((i,j), (r,dr)) -> float
        maxhopradius = 0.1,
        hopradius = 0.03,                   # hop shader, accepts ((i,j), (r,dr)) -> float
        hopoffset = 1.0,
        hopdarken = 0.85,
