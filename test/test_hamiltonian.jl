...
@testset "basic hamiltonians" begin
    ...
    for preset in presets, type in types, lat in (preset(; type), supercell(preset(; type)))
        ...
        for (t, o) in zip(ts, orbs)
            o´ = ifelse(o === Val(2) && preset === LatticePresets.honeycomb, (Val(2), Val(1)), o)
    ...
    h = LatticePresets.square() |> supercell(3) |> hamiltonian(hopping(1, indices = (1:8 .=> 2:9, 9=>1), range = 3, plusadjoint = true))
...
@testset "hamiltonian wrap" begin
    ...
    @test wh(()) ≈ h(SA[1,2,3])
    wh = wrap(h, (1,:,3))
    @test wh(SA[2]) ≈ h(SA[1,2,3])
    wh = wrap(h, (1,:,:))
    @test wh(SA[2,3]) ≈ h(SA[1,2,3])
    h = LatticePresets.bcc() |> hamiltonian(hopping((r, dr) -> 1/norm(dr), range = 10)) |> supercell(3)
    wh = wrap(h, (1,2,3))
    @test wh(()) ≈ h(SA[1,2,3])
    wh = wrap(h, (1,:,3))
    @test wh(SA[2]) ≈ h(SA[1,2,3])
...
#     notscalar(s::Integer, h) = s:s
#     notscalar(s::Quantica.SiteSelector, h) = collect(siteindices(h, s))
#     notscalar(s, h) = s
