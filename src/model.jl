...
onsite(m::TightbindingModel; kw...) = TightbindingModel(
    Any[OnsiteTerm(o, siteselector(o.selector; kw...)) for o in terms(m) if o isa OnsiteTerm])
...
hopping(m::TightbindingModel; kw...) = TightbindingModel(
    Any[HoppingTerm(t, hopselector(t.selector; kw...)) for t in terms(m) if t isa HoppingTerm])
...
apply(m::TightbindingModel, lat::Lattice) = TightbindingModel(Any[apply(t, lat) for t in terms(m)])
...
Base.:*(x::Number, m::TightbindingModel) = TightbindingModel(Any[x * t for t in terms(m)])
Base.:*(m::TightbindingModel, x::Number) = x * m
Base.:-(m::TightbindingModel) = (-1) * m

Base.:+(m::TightbindingModel, m´::TightbindingModel) = TightbindingModel(Any[terms(m)..., terms(m´)...])
...
Base.adjoint(m::TightbindingModel) = TightbindingModel(Any[adjoint(t) for t in terms(m)])
