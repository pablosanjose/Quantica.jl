...
struct SiteModifier{N,S<:Selector,F<:ParametricFunction{N}} <: ElementModifier{N,S,F}
    f::F
    selector::S
end

struct HopModifier{N,S<:Selector,F<:ParametricFunction{N}} <: ElementModifier{N,S,F}
    f::F
    selector::S
end
...
macro onsite!(kw, f)
    ...
    return esc(:(Quantica.SiteModifier(Quantica.ParametricFunction{$N}($f, $(Val(params))), Quantica.siteselector($kw))))
end

macro onsite!(f)
    f, N, params = get_f_N_params(f, "Only @onsite!(args -> body; kw...) syntax supported.  Mind the `;`.")
    return esc(:(Quantica.SiteModifier(Quantica.ParametricFunction{$N}($f, $(Val(params))), Quantica.siteselector())))
end
...
macro hopping!(kw, f)
    ...
    return esc(:(Quantica.HopModifier(Quantica.ParametricFunction{$N}($f, $(Val(params))), Quantica.hopselector($kw))))
end

macro hopping!(f)
    f, N, params = get_f_N_params(f, "Only @hopping!(args -> body; kw...) syntax supported. Mind the `;`.")
    return esc(:(Quantica.HopModifier(Quantica.ParametricFunction{$N}($f, $(Val(params))), Quantica.hopselector())))
end
...
resolve(o::SiteModifier, lat) = SiteModifier(o.f, resolve(o.selector, lat))

resolve(h::HopModifier, lat) = HopModifier(h.f, resolve(h.selector, lat))
...
@inline (o!::UniformSiteModifier)(o, r; kw...) = o!(o; kw...)
@inline (o!::UniformSiteModifier)(o, r, dr; kw...) = o!(o; kw...)
@inline (o!::UniformSiteModifier)(o; kw...) = o!.f(o; kw...)
@inline (o!::SiteModifier)(o, r, dr; kw...) = o!(o, r; kw...)
@inline (o!::SiteModifier)(o, r; kw...) = o!.f(o, r; kw...)

@inline (h!::UniformHopModifier)(t, r, dr; kw...) = h!(t; kw...)
@inline (h!::UniformHopModifier)(t; kw...) = h!.f(t; kw...)
@inline (h!::HopModifier)(t, r, dr; kw...) = h!.f(t, r, dr; kw...)
